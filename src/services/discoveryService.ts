...
const FALLBACK_COMPANIES = [
  'nycballet.com', 'bostonballet.org', 'pnb.org', 'washingtonballet.org',
  'atlantaballet.com', 'houstonballet.org', 'joffrey.org', 'coloradoballet.org',
  'miamicityballet.org', 'balletaz.org', 'balletwest.org', 'pittsburghballet.org',
  'balletmemphis.org', 'kcballet.org', 'oregonballet.org', 'balletmet.org',
  'richmondballet.com', 'balletcharlotte.org', 'ballet-oklahoma.org', 'sarasotaballet.org'
];
...
export async function discoverFromWebSearch(): Promise<string[]> {
  const queries = ["Nutcracker+ballet+tickets+2026", "Nutcracker+performance+December+2026"];
  const urls: string[] = [];
  ...
  for (const query of queries) {
    try {
      ...
      const standardMatches = html.matchAll(/<a\s+[^>]*class="result__a"\s+[^>]*href="([^"]+)"/gi);
      const testIdMatches = html.matchAll(/<a\s+[^>]*data-testid="result-title-a"\s+[^>]*href="([^"]+)"/gi);
      const genericMatches = html.matchAll(/<a\s+[^>]*href="(https?:\/\/[^"]+)"/gi);
...
function isValidDomain(url: string): boolean {
  try {
    ...
    const excluded = ['ticketmaster.', 'stubhub.', 'viagogo.', 'google.', 'duckduckgo.', 'facebook.', 'instagram.', 'twitter.', 'youtube.'];
    return !excluded.some(ex => d.includes(ex)) &&
           (d.endsWith('.org') || d.endsWith('.com') || d.endsWith('.edu') || d.endsWith('.gov'));
  } catch { return false; }
}

export async function discoverFromBalletDirectories(): Promise<string[]> {
  const urls: string[] = [];
  const basePatterns = ['/nutcracker', '/productions/nutcracker', '/season/nutcracker', '/performances/nutcracker'];
  ...
  try {
    ...
    const html = await response.text();
    const domainMatches = html.matchAll(/https?:\/\/(www\.)?([a-zA-Z0-9-]+\.(org|com))/g);

    companies = Array.from(new Set(Array.from(domainMatches).map(m => `https://${m[2]}`))).slice(0, 30);
  } catch (e) {
...
export async function validateEventPage(url: string) {
  try {
    ...
    return { valid: matches.length >= 1, confidence: (matches.length / keywords.length) * 100, html };
  } catch {
    return { valid: false };
  }
}
 ...
 */
export function extractEventMetadata(url: string, html: string): Omit<Event, "id" | "created_at"> {
  ...
  return {
    ...
    // Required fields with defaults to fix TypeScript build error
